...
<div class="research-canvas ir-shell" id="interview-readiness">
  ...
  <section class="research-canvas__method" data-reveal>
    <div><p class="research-canvas__eyebrow">Interview loop</p><h2>Refresh, explain, pressure-test, review.</h2></div>
    <ol>
      <li><span>01</span><strong>Refresh</strong><p>Open the linked Lab page and rebuild the process or architecture model.</p></li>
      <li><span>02</span><strong>Explain</strong><p>Answer one question without notes. Keep the business goal and ownership visible.</p></li>
      <li><span>03</span><strong>Pressure-test</strong><p>Use Interview Mode or the Assessment Lab. Let follow-up questions expose weak boundaries.</p></li>
      <li><span>04</span><strong>Review</strong><p>Update the roadmap and one project story. Your next session should change because of the result.</p></li>
    </ol>
  </section>

  <section class="research-canvas__inventory" data-reveal>
    <header><p class="research-canvas__eyebrow">Evidence</p><h2>A good answer needs more than correct terminology.</h2><p>For senior roles, prepare evidence that shows what you did, what you decided, and what changed.</p></header>
    <div class="ir-grid">
      <article class="ir-card"><p class="ir-kicker">Knowledge</p><h3>Can I explain it?</h3><p>Process, data, rules, systems, and boundaries in plain language.</p></article>
      <article class="ir-card"><p class="ir-kicker">Experience</p><h3>Have I worked with it?</h3><p>A concrete project, incident, rollout, design discussion, or operational responsibility.</p></article>
      <article class="ir-card"><p class="ir-kicker">Decision</p><h3>Can I defend a trade-off?</h3><p>Why one option was chosen, what it cost, and what risk remained.</p></article>
      <article class="ir-card"><p class="ir-kicker">Outcome</p><h3>What changed?</h3><p>Business effect, operational improvement, delivery result, risk reduction, or a clear lesson.</p></article>
    </div>
    <p><a class="ir-link" href="/labs/interview-readiness/stories/">Build the story bank <span class="material-symbols-outlined" aria-hidden="true">arrow_forward</span></a></p>
  </section>
</div>
...
(() => {
  ...
  if (!IR) return;
  const overall = document.getElementById('ir-overall');
  ...
  function render() {
    ...
    Object.entries(data.tracks).forEach(([id,track]) => {
      ...
      card.innerHTML = `<div class="ir-track__head"><strong>${track.label}</strong><span>${track.readiness}%</span></div><div class="ir-track__bar" aria-hidden="true"><span style="width:${track.readiness}%"></span></div><small>${track.count} topics</small>`;
      tracks.appendChild(card);
    });

    daily.replaceChildren();
    IR.dailyPlan(4).forEach((item,index) => {
      const row = document.createElement('article');
      row.className = 'ir-daily__item';
      const action = item.status.score === 0 ? 'Refresh' : item.status.score === 1 ? 'Explain' : 'Defend';
