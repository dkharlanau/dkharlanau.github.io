...
Take one of the previous labs and treat it like a service that another engineer must operate on Monday morning. The goal is to remove hidden assumptions from the demo.
...
The service must have a stable API, versioned configuration, synthetic test data, and no private credentials in the repository.
...
```json
{
  ...
  "prompt_version": "ops-12",
  "tool_schema_version": "workspace-tools-5",
  "retrieval_version": "knowledge-index-2026-08-15",
...
One trace should connect request, retrieval, model calls, tool/MCP calls, auth checks, approvals, retries, and final result.
...
|---|---|
| model timeout | retry if safe, then controlled failure/degraded response |
| retrieval unavailable | use allowed trusted tools or return evidence gap |
| read API unavailable | partial result with missing dependency named |
| permission denied | stop that path, do not reinterpret as missing data |
| rate limit | backoff, respect total request budget |
| write timeout | check idempotency/request ID before retry |
| approval expired | require new approval |

## Step 6: secrets and identity

Use environment-specific secret storage or workload identity. Development credentials must not reach production. A model should receive task data, not backend credentials.
...
write service down -> investigation remains read-only
```
...
Practice rolling back code version, prompt bundle, model profile, retrieval index, tool/MCP server, and policy configuration.
...
4. Which eval protects this behavior?
