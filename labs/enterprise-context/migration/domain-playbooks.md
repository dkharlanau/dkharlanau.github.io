...
<div class="research-canvas">
  ...
  <section class="research-canvas__boundary" data-reveal>
    ...
    <p><strong>Open-document rules are object- and approach-specific.</strong> Current staging objects for open sales and purchase orders carry initial open state, not an old executed document flow. S/4HANA 2025 also provides direct-transfer objects with their own source selection. Other document families can behave differently: for example, a direct-transfer sales contract can migrate the remaining quantity of a partially processed contract. Never turn one object's restriction into a universal migration rule.</p>
    <p><strong>Architectural consequence.</strong> When an executed legacy chain is not supported by the selected migration object, retain the executed chain as history and create only the remaining business commitment in the target under an approved cutover rule. Do not rebuild historical receipts, deliveries, invoices or confirmations only to imitate the old document flow.</p>
  </section>

  <section class="research-canvas__inventory" id="waves" data-reveal>
    <header><p class="research-canvas__eyebrow">Load waves</p><h2>Build the target in dependency order.</h2><p>Validate the final order against the migration-object prerequisites for the exact target release and migration approach.</p></header>
    <div class="research-route-list">
      ...
      <a id="wave-3" href="#wave-3"><span>3</span><strong>Eligible open business</strong><small>Only transaction state supported by the exact migration object and approach, plus approved target recreation of remaining commitments where legacy execution history cannot move.</small><i class="material-symbols-outlined" aria-hidden="true">pending_actions</i></a>
  ...
  <section class="research-canvas__inventory" id="sales" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="/labs/enterprise-context/migration/object-catalog/#open-business"><span>SD4</span><strong>Open sales orders</strong><small>Use the released object only for the open state it supports. Staging and direct-transfer content can have different selection and scope rules, so freeze the rule against the exact target release.</small><i class="material-symbols-outlined" aria-hidden="true">shopping_cart</i></a>
      <a href="#sales-partial"><span>SD5</span><strong>Partially executed order</strong><small>If the selected object excludes the executed chain, keep that chain in legacy history and recreate only the remaining customer commitment in the target. Do not assume the rule for sales orders also applies to contracts or every direct-transfer object.</small><i class="material-symbols-outlined" aria-hidden="true">call_split</i></a>
  ...
  <section class="research-canvas__inventory" id="sales-partial" data-reveal>
    <header><p class="research-canvas__eyebrow">Sales decision</p><h2>Separate executed history from the remaining promise.</h2></header>
    <div class="research-route-list">
      <a href="#sales-partial"><span>A</span><strong>Eligible open order state</strong><small>Use the released migration object and validate its migration approach, document type, selection, field and status restrictions.</small><i class="material-symbols-outlined" aria-hidden="true">check_circle</i></a>
      <a href="#sales-partial"><span>B</span><strong>Executed flow is out of scope</strong><small>Retain deliveries, billing and other executed documents as history. Create only the remaining target commitment when the business still needs it.</small><i class="material-symbols-outlined" aria-hidden="true">history</i></a>
      <a href="#sales-partial"><span>C</span><strong>Object-specific exception</strong><small>Some migration objects have their own partial-processing logic. S/4HANA direct-transfer sales-contract content, for example, can migrate the remaining quantity of a partially processed contract. Read the object documentation instead of reusing the sales-order rule.</small><i class="material-symbols-outlined" aria-hidden="true">difference</i></a>
      <a href="#sales-partial"><span>D</span><strong>Complex scenarios</strong><small>Returns, intercompany, third-party and configurable products need an explicit cutover rule and target process test; do not assume the generic order object covers them.</small><i class="material-symbols-outlined" aria-hidden="true">warning</i></a>
    </div>
  </section>
  ...
  <section class="research-canvas__inventory" id="procurement" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="/labs/enterprise-context/migration/object-catalog/#open-business"><span>MM3</span><strong>Open purchase orders</strong><small>The staging open-PO object is for items that have not yet been delivered and invoiced. Direct transfer has its own selection logic. Confirm the exact approach before deciding whether to close, split, recreate or migrate a source PO.</small><i class="material-symbols-outlined" aria-hidden="true">shopping_bag</i></a>
      <a href="#procurement-grir"><span>MM4</span><strong>Remaining commitment</strong><small>If purchasing activity must continue but the old execution chain cannot move, recreate the required target PO and handle historical GR/IR, receipts and invoices through the agreed finance/history boundary.</small><i class="material-symbols-outlined" aria-hidden="true">compare_arrows</i></a>
  ...
  <section class="research-canvas__inventory" id="procurement-grir" data-reveal>
    ...
    <div class="research-route-list">
      <a href="#procurement-grir"><span>PO</span><strong>Operational remainder</strong><small>Create only the commitment that still needs target execution when the selected migration object cannot carry the old execution state; do not recreate historical goods movements.</small><i class="material-symbols-outlined" aria-hidden="true">pending</i></a>
  ...
  <section class="research-canvas__inventory" id="production" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="/labs/enterprise-context/migration/object-catalog/#release-watchlist"><span>PP4</span><strong>Open production orders</strong><small>Use the exact status contract for the target object. Public Edition 2608 PP_PRODNORD accepts source orders in Created or Released status and creates target orders in status Created. Do not generalise that behaviour to another release or approach.</small><i class="material-symbols-outlined" aria-hidden="true">manufacturing</i></a>
  ...
  <section class="research-canvas__inventory" id="maintenance-projects" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="/labs/enterprise-context/migration/object-catalog/#release-watchlist"><span>PM</span><strong>Maintenance plans and open work</strong><small>Close completed legacy work and verify support before carrying open order state. Public Edition 2608 maintenance-order migration uses internal numbering, so preserve a legacy cross-reference where old order numbers matter.</small><i class="material-symbols-outlined" aria-hidden="true">handyman</i></a>
