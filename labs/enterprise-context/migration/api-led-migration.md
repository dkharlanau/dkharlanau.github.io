...
<div class="research-canvas">
  ...
  <section class="research-canvas__boundary" data-reveal>
    ...
    <p><strong>Default:</strong> if SAP provides a migration object that fits the initial state, use Migration Cockpit unless there is a clear reason not to. It already gives migration-specific mapping, simulation, object processing and migration status.</p>
    <p><strong>Use an API deliberately.</strong> An API normally executes the live business object. It can run pricing, checks, derivations, status logic, output, workflow or accounting behaviour that is different from a migration object.</p>
  ...
  <section class="research-canvas__inventory" id="decision" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="#transactions"><span>DOC</span><strong>Open transaction must be recreated</strong><small>Use an API only when creating a normal target document is the intended business result. Do not assume that legacy status, document flow or partial processing can be reconstructed.</small><i class="material-symbols-outlined" aria-hidden="true">receipt_long</i></a>
      ...
      <a href="/labs/enterprise-context/migration/cutover/#history"><span>HIST</span><strong>Closed historical documents</strong><small>Do not recreate history through transactional APIs by default. Keep history in the agreed legacy/archive/data platform unless a supported target requirement justifies migration.</small><i class="material-symbols-outlined" aria-hidden="true">history</i></a>
    </div>
  </section>
  ...
  <section class="research-canvas__inventory" id="architecture" data-reveal>
    ...
    <div class="research-route-list">
      <a href="#snapshot"><span>01</span><strong>Source snapshot</strong><small>Freeze or timestamp the selected source population. Store source keys, extraction timestamp and control totals.</small><i class="material-symbols-outlined" aria-hidden="true">download</i></a>
      <a href="#canonical"><span>02</span><strong>Canonical migration layer</strong><small>Cleanse, map, split, merge and enrich data before it reaches the API adapter. Keep transformations versioned.</small><i class="material-symbols-outlined" aria-hidden="true">transform</i></a>
  ...
  <section class="research-canvas__inventory" id="contract" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="#contract"><span>FX</span><strong>Side effects</strong><small>Check pricing, ATP, credit, workflow, output, tax, accounting, change documents and other logic that can run when a normal business object is created.</small><i class="material-symbols-outlined" aria-hidden="true">bolt</i></a>
  ...
  <section class="research-canvas__inventory" id="examples" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="https://help.sap.com/docs/SAP_S4HANA_CLOUD/03c04db2a7434731b7fe21dca77440da/00d244581efca007e10000000a441470.html" target="_blank" rel="noopener"><span>SO</span><strong>Sales Order — API_SALES_ORDER_SRV</strong><small>The OData V2 API can create and change sales orders. Current operations documentation also limits a batch changeset to one sales order. That concrete rule matters for throughput design.</small><i class="material-symbols-outlined" aria-hidden="true">shopping_cart</i></a>
      <a href="https://help.sap.com/docs/SAP_S4HANA_CLOUD/03c04db2a7434731b7fe21dca77440da/c09676370b0146ba976081e3da23a692.html" target="_blank" rel="noopener"><span>ETAG</span><strong>Sales Order concurrency</strong><small>SAP documents ETag handling for relevant Sales Order create/update/delete operations. A migration delta process must respect current target state instead of overwriting concurrent changes.</small><i class="material-symbols-outlined" aria-hidden="true">compare_arrows</i></a>
  ...
  <section class="research-canvas__inventory" id="stock" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="https://help.sap.com/docs/SAP_S4HANA_CLOUD/3f57e7df4a114edabffe8b2d581a59ed/1aef4e402acd4c8b8ec2ea2bfda7715b.html" target="_blank" rel="noopener"><span>POST</span><strong>Material Document API posts movements</strong><small>API_MATERIAL_DOCUMENT_SRV can create and cancel material documents. Using it for opening stock is a posting design, not a neutral copy operation. Movement type, valuation, batch/serial data and accounting reconciliation must be explicit.</small><i class="material-symbols-outlined" aria-hidden="true">swap_horiz</i></a>
  ...
  <section class="research-canvas__inventory" id="performance" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="#performance"><span>3</span><strong>Find safe concurrency</strong><small>Increase parallelism until throughput stops improving or errors/receiver pressure rise. Use that measured level, not the maximum thread count of the loader.</small><i class="material-symbols-outlined" aria-hidden="true">speed</i></a>
  ...
  <section class="research-canvas__inventory" id="delta" data-reveal>
    ...
    <div class="research-route-list">
      <a href="#snapshot"><span>T0</span><strong>Take a controlled baseline</strong><small>Record extraction timestamp and population rules. Baseline counts and sums become reconciliation evidence.</small><i class="material-symbols-outlined" aria-hidden="true">photo_camera</i></a>
      ...
      <a href="#contract"><span>U</span><strong>Classify delta operations</strong><small>Create-new, update-existing and delete/cancel are different operations. Confirm that the target API supports the required change semantics.</small><i class="material-symbols-outlined" aria-hidden="true">rule</i></a>
  ...
  <section class="research-canvas__inventory" id="reconciliation" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="/labs/enterprise-context/migration/cutover/#business-proof"><span>E2E</span><strong>Process proof</strong><small>Run representative sales, procurement, production, warehouse and finance scenarios against migrated data before go/no-go.</small><i class="material-symbols-outlined" aria-hidden="true">play_circle</i></a>
    </div>
  </section>
  ...
  <section class="research-canvas__inventory" id="assessment" data-reveal>
    ...
    <div class="research-route-list">
      ...
      <a href="#decision"><span>2</span><strong>Check Migration Cockpit first</strong><small>If a released migration object fits, explain why a custom/API route is still justified.</small><i class="material-symbols-outlined" aria-hidden="true">fact_check</i></a>
      <a href="#contract"><span>3</span><strong>Prove API semantics</strong><small>Create/update support, numbering, side effects, custom fields, batch limits and security must fit the requirement.</small><i class="material-symbols-outlined" aria-hidden="true">api</i></a>
