...
EVIDENCE_PROFILE_PATH = ASSESSMENT_DATA / "evidence-profile.json"
...
def page_sources() -> dict[str, dict[str, Any]]:
    ...
    for path in sorted((ROOT / "labs").rglob("*")):
        ...
        if isinstance(permalink, str) and permalink.startswith("/labs/"):
...
def collect_catalog_routes(value: Any, result: set[str]) -> None:
    if isinstance(value, str):
        if value.startswith("/labs/") and not value.startswith("/labs/assessment/data/") and not re.search(r"\.(?:json|jsonl|ya?ml|txt)$", value):
            result.add(value)
...
def all_catalog_routes() -> list[str]:
    ...
    collect_catalog_routes(load_json(CATALOG_PATH), result)
    return sorted(result)


def evidence_profiles() -> dict[str, Any]:
    payload = load_json(EVIDENCE_PROFILE_PATH) if EVIDENCE_PROFILE_PATH.exists() else {}
    default = payload.get("defaults", {}).get("enterprise_context", {
        "expected_evidence_classes": ["sap_product_primary", "author_heuristic"],
        "external_review_mode": "required_for_load_bearing_product_claims",
        "counts_as_source_review_debt": True,
    })
    return {"default": default, "overrides": payload.get("route_overrides", {})}


def route_evidence_profile(route: str, profiles: dict[str, Any]) -> dict[str, Any]:
    override = profiles["overrides"].get(route)
    if override:
        return override
...
def factual_review_index() -> dict[str, dict[str, Any]]:
    ...
    claims_by_id = {str(item.get("id")): item for item in payload.get("claims", []) if item.get("id")}
    ...
    for route in payload.get("routes", []):
        ...
        result[route_id] = {
            ...
            "evidence_classes": sorted({item.get("evidence_class", "sap_product_primary") for item in route_claims}),
...
def structural_checks(frontmatter: dict[str, Any], body: str) -> dict[str, bool]:
    ...
    return {
        "frontmatter_complete": all(bool(frontmatter.get(key)) for key in ("title", "description", "permalink", "last_modified_at")) and isinstance(tags, list) and bool(tags),
...
def readiness_state(frontmatter: dict[str, Any], checks: dict[str, bool]) -> str:
    ...
    draft_boundary = status == "draft" or "noindex" in robots or frontmatter.get("sitemap") is False
    if not draft_boundary:
        return "public_or_indexable"
    return "human_review_candidate" if sum(checks.values()) >= 4 else "needs_structure"


def review_priority(state: str, factual: dict[str, Any], profile: dict[str, Any]) -> tuple[str, str]:
    ...
    if external_required and factual_status in {"not_reviewed", "needs_source_review"}:
        return "P0", "Structure is mature but load-bearing external claims have not completed the required primary-source review."
    ...
    if not external_required:
        return "P2", "This route uses selective external evidence and explicit author heuristics; source review is not the primary promotion gate."
    return "P2", "Human review is the next action."
...
def audit() -> dict[str, Any]:
    ...
    for route in all_catalog_routes():
        factual = factual_index.get(route, {"status": "not_reviewed", "claim_count": 0, "source_supported_count": 0, "source_conflict_count": 0, "release_scope_unclear_count": 0, "evidence_classes": [], "reviewed_at": None, "human_verification_required": True})
        profile = route_evidence_profile(route, profiles)
        page = pages.get(route)
        if page is None:
            items.append({"route": route, "source_path": None, "state": "missing_source", "priority": "P2", "review_reason": "Catalog route cannot be resolved to a Lab source file.", "structural_score": 0, "checks": {}, "evidence_profile": profile, "factual_review": factual, "verified": False, "status": None, "robots": None, "sitemap": None, "body_characters": 0, "internal_link_count": 0, "machine_link_count": 0})
            continue
        frontmatter, body, path = page["frontmatter"], page["body"], page["path"]
        checks = structural_checks(frontmatter, body)
        links = internal_links(body)
        state = readiness_state(frontmatter, checks)
        priority, reason = review_priority(state, factual, profile)
        items.append({
            "route": route,
    ...
    items.sort(key=lambda item: (state_order[item["state"]], priority_order[item["priority"]], factual_order.get(item["factual_review"]["status"], 9), -item["structural_score"], item["route"]))
    counts: dict[str, int] = {}
    factual_counts: dict[str, int] = {}
    priority_counts: dict[str, int] = {}
    profile_counts = {"source_review_debt_applicable": 0, "selective_or_heuristic": 0}
    for item in items:
        ...
        if item["evidence_profile"].get("counts_as_source_review_debt", False):
            profile_counts["source_review_debt_applicable"] += 1
        else:
            profile_counts["selective_or_heuristic"] += 1
    return {
        ...
        "evidence_profile_counts": profile_counts,
...
def main() -> int:
    ...
    if args.check:
        if not OUTPUT_PATH.exists() or OUTPUT_PATH.read_text(encoding="utf-8") != rendered:
            print(f"Stale or missing promotion readiness inventory: {OUTPUT_PATH.relative_to(ROOT)}")
            return 2
        print(f"Promotion readiness inventory is current: {payload['scope_route_count']} routes, {payload['counts'].get('human_review_candidate', 0)} human-review candidate(s), {payload['factual_review_counts'].get('source_supported', 0)} source-reviewed route(s).")
        return 0
    OUTPUT_PATH.write_text(rendered, encoding="utf-8")
    print(f"Generated {OUTPUT_PATH.relative_to(ROOT)}: {payload['scope_route_count']} routes, {payload['counts'].get('human_review_candidate', 0)} human-review candidate(s), {payload['factual_review_counts'].get('source_supported', 0)} source-reviewed route(s).")
    return 0
