#!/usr/bin/env python3
"""
Export selected agent skills into .claude/skills/ for Claude Code.

Usage:
    python3 agent-skills/exporters/export_claude_skills.py --profile sap-ams
    python3 agent-skills/exporters/export_claude_skills.py --profile integration-architecture
...
then copies the matching skills into .claude/skills/<skill-name>/.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export agent skills to .claude/skills/")
    ...
    print(f"Exporting {len(skills_to_export)} skill(s) to {CLAUDE_SKILLS_DIR}...")
