...
def test_manifest_json_is_valid():
    ...
    assert data["unverified_count"] == 58
    assert len(data["entries"]) == 72
...
def test_related_json_is_valid():
    ...
    assert data["count"] == 223
    assert data["broken_link_count"] == 0
    assert data["warnings"] == []
    assert len(data["edges"]) == 223
...
def test_dynamic_discovery_returns_22_article_pages():
    ...
    assert len(articles) == 72, f"Expected 72 articles, found {len(articles)}"
