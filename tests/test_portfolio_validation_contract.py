...
def test_validated_status_requires_independent_task_evidence_not_self_proof():
    ...
    assert {"author_run_demo", "ci_or_regression_tests", "self_or_agent_review", "successful_release_pipeline", "documentation_or_pages_site"} <= non_qualifying
...
def test_validation_records_keep_negative_results_scope_and_limit_claims():
    ...
    assert {"product", "product_version_or_commit", "participant_independence", "task", "success_criteria", "input_class", "outcome", "defects_or_ambiguities", "limitations"} <= required
