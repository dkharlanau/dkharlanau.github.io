---
layout: blog
title: "How to Design Resilience and Disaster Recovery for SAP Integrations Without Creating Duplicate Business Transactions"
description: "A delivery that had already been cancelled becomes active again because an older message was replayed."
slug: how-to-design-resilience-and-disaster-recovery-for-sap-integrations
permalink: /blog/how-to-design-resilience-and-disaster-recovery-for-sap-integrations/
date: 2026-07-17
...
tags:
  ...
  - integration
canonical_url: https://dkharlanau.github.io/blog/how-to-design-resilience-and-disaster-recovery-for-sap-integrations/
status: needs_verification
verified: false
robots: noindex,follow
sitemap: false
reading_time_minutes: 31
migration_sequence: 32
...
- [Begin with business criticality](#begin-with-business-criticality)
- [Define RTO and RPO in business terms](#define-rto-and-rpo-in-business-terms)
- [Map the critical path](#map-the-critical-path)
- [The most dangerous state is an uncertain outcome](#the-most-dangerous-state-is-an-uncertain-outcome)
- [Idempotency is the foundation of safe recovery](#idempotency-is-the-foundation-of-safe-recovery)
- [Retry only what may improve](#retry-only-what-may-improve)
- [Circuit breakers protect the recovering system](#circuit-breakers-protect-the-recovering-system)
- [Queues provide resilience but create backlog risk](#queues-provide-resilience-but-create-backlog-risk)
- [Backpressure is a business decision](#backpressure-is-a-business-decision)
- [Disaster recovery needs an active-state model](#disaster-recovery-needs-an-active-state-model)
- [Failover and failback are different risks](#failover-and-failback-are-different-risks)
- [Configuration is part of the recoverable state](#configuration-is-part-of-the-recoverable-state)
- [Idempotency storage is a critical recovery dependency](#idempotency-storage-is-a-critical-recovery-dependency)
- [Exactly once is a business outcome, not a transport slogan](#exactly-once-is-a-business-outcome-not-a-transport-slogan)
- [Recovery needs compensation, not database rollback](#recovery-needs-compensation-not-database-rollback)
- [A saga needs state and ownership](#a-saga-needs-state-and-ownership)
- [Recovery order matters](#recovery-order-matters)
- [File recovery requires sequence control](#file-recovery-requires-sequence-control)
- [Events require publisher and consumer recovery](#events-require-publisher-and-consumer-recovery)
- [Dead-letter queues need business triage](#dead-letter-queues-need-business-triage)
- [Manual fallback must be designed in advance](#manual-fallback-must-be-designed-in-advance)
- [Read-only degraded mode can be safer than full fallback](#read-only-degraded-mode-can-be-safer-than-full-fallback)
- [Partner recovery requires explicit agreements](#partner-recovery-requires-explicit-agreements)
- [Recovery testing must include business effects](#recovery-testing-must-include-business-effects)
- [Disaster-recovery runbooks should be transaction aware](#disaster-recovery-runbooks-should-be-transaction-aware)
- [Prioritize recovery by business impact](#prioritize-recovery-by-business-impact)
- [A reference resilience architecture](#a-reference-resilience-architecture)
- [Example: resilient customer-order integration](#example-resilient-customer-order-integration)
- [Example: resilient payment integration](#example-resilient-payment-integration)
- [Example: resilient goods-movement integration](#example-resilient-goods-movement-integration)
...
- [The goal is not zero failure](#the-goal-is-not-zero-failure)
...
- Previous in the migration: [Why Green Interfaces Still Produce Broken Business Processes: Observability and Reconciliation in SAP Integration Architecture](/blog/why-green-interfaces-still-produce-broken-business-processes/)
- Next in the migration: [Why One Technical User Can Defeat Your Entire SAP Integration Security Architecture](/blog/why-one-technical-user-can-defeat-your-entire-sap-integration-security/)
