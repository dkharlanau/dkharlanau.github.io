---
layout: blog
title: "How to Modernize SAP IDoc and File Integrations Without Rewriting the Entire Landscape"
description: "analysis; - redesign; - development; - testing; - business validation; - cutover; - hypercare."
slug: how-to-modernize-sap-idoc-and-file-integrations-without-rewriting-the
permalink: /blog/how-to-modernize-sap-idoc-and-file-integrations-without-rewriting-the/
date: 2026-07-17
...
tags:
  ...
  - integration
  - sap-architecture
canonical_url: https://dkharlanau.github.io/blog/how-to-modernize-sap-idoc-and-file-integrations-without-rewriting-the/
status: needs_verification
verified: false
robots: noindex,follow
sitemap: false
reading_time_minutes: 30
migration_sequence: 27
...
- [Old protocol does not automatically mean bad architecture](#old-protocol-does-not-automatically-mean-bad-architecture)
- [Why IDocs remain common](#why-idocs-remain-common)
- [Why files remain common](#why-files-remain-common)
- [Modernization begins with discovery](#modernization-begins-with-discovery)
- [Identify whether the interface is still used](#identify-whether-the-interface-is-still-used)
- [Build a dependency map](#build-a-dependency-map)
- [Classify the modernization decision](#classify-the-modernization-decision)
- [1. Retire](#1-retire)
- [2. Retain](#2-retain)
- [3. Rehost](#3-rehost)
- [4. Replatform](#4-replatform)
- [5. Wrap](#5-wrap)
- [6. Redesign](#6-redesign)
- [7. Replace with standard content](#7-replace-with-standard-content)
- [8. Consolidate](#8-consolidate)
- [Migration value differs by category](#migration-value-differs-by-category)
- [Do not redesign before understanding current behaviour](#do-not-redesign-before-understanding-current-behaviour)
- [Extract the business contract from the old interface](#extract-the-business-contract-from-the-old-interface)
- [Example: an old order IDoc](#example-an-old-order-idoc)
- [The strangler pattern for integrations](#the-strangler-pattern-for-integrations)
- [Example: customer master distribution](#example-customer-master-distribution)
- [Anti-corruption layers protect new consumers](#anti-corruption-layers-protect-new-consumers)
- [Wrapping is not the same as solving](#wrapping-is-not-the-same-as-solving)
- [A facade should report honest states](#a-facade-should-report-honest-states)
- [Keep business identity across old and new paths](#keep-business-identity-across-old-and-new-paths)
- [Modernizing files does not always mean removing files](#modernizing-files-does-not-always-mean-removing-files)
- [Add a control record](#add-a-control-record)
- [Distinguish full snapshots and deltas](#distinguish-full-snapshots-and-deltas)
- [File arrival is not business completion](#file-arrival-is-not-business-completion)
- [When an IDoc should be retained](#when-an-idoc-should-be-retained)
- [When an IDoc should be wrapped](#when-an-idoc-should-be-wrapped)
- [When an IDoc should be redesigned](#when-an-idoc-should-be-redesigned)
- [When a file should be retained](#when-a-file-should-be-retained)
- [When a file should become an API](#when-a-file-should-become-an-api)
- [When a file should become an event stream](#when-a-file-should-become-an-event-stream)
- [Rehost versus redesign](#rehost-versus-redesign)
- [Rehost](#rehost)
- [Redesign](#redesign)
- [SAP’s current migration position supports phased modernization](#sap-s-current-migration-position-supports-phased-modernization)
- [Build a migration decision score](#build-a-migration-decision-score)
- [Business criticality](#business-criticality)
- [Technical risk](#technical-risk)
- [Change demand](#change-demand)
- [Coupling](#coupling)
- [Operational quality](#operational-quality)
- [Modernization value](#modernization-value)
- [Avoid scoring only technical age](#avoid-scoring-only-technical-age)
- [Create modernization waves](#create-modernization-waves)
- [Start with common capabilities](#start-with-common-capabilities)
- [Migration factories can scale good and bad designs](#migration-factories-can-scale-good-and-bad-designs)
- [Use automated assessment carefully](#use-automated-assessment-carefully)
- [Mapping reuse should be governed](#mapping-reuse-should-be-governed)
- [Preserve traceability during dual operation](#preserve-traceability-during-dual-operation)
- [Use shadow mode before active cutover](#use-shadow-mode-before-active-cutover)
- [Shadow mode has limitations](#shadow-mode-has-limitations)
- [Parallel run needs duplicate protection](#parallel-run-needs-duplicate-protection)
- [Reconcile outputs, not only payloads](#reconcile-outputs-not-only-payloads)
- [Define acceptable differences](#define-acceptable-differences)
- [Test production history](#test-production-history)
- [Build golden datasets](#build-golden-datasets)
- [Cutover needs an in-flight strategy](#cutover-needs-an-in-flight-strategy)
- [Establish a cutover boundary](#establish-a-cutover-boundary)
- [Plan rollback honestly](#plan-rollback-honestly)
- [Do not decommission too early](#do-not-decommission-too-early)
- [Do not keep old runtime indefinitely](#do-not-keep-old-runtime-indefinitely)
- [Hybrid integration is a transition and operating model](#hybrid-integration-is-a-transition-and-operating-model)
- [Security modernization may justify migration even when the process stays the same](#security-modernization-may-justify-migration-even-when-the-process-stays-the-same)
- [Modern monitoring should combine technical and business signals](#modern-monitoring-should-combine-technical-and-business-signals)
- [Support processes must be migrated too](#support-processes-must-be-migrated-too)
- [Compare total cost, not only development effort](#compare-total-cost-not-only-development-effort)
- [A practical modernization decision table](#a-practical-modernization-decision-table)
- [A practical programme sequence](#a-practical-programme-sequence)
- [Phase 1: Discover](#phase-1-discover)
- [Phase 2: Remove](#phase-2-remove)
- [Phase 3: Stabilize](#phase-3-stabilize)
- [Phase 4: Create standards](#phase-4-create-standards)
- [Phase 5: Build shared foundations](#phase-5-build-shared-foundations)
- [Phase 6: Replatform low-risk flows](#phase-6-replatform-low-risk-flows)
- [Phase 7: Wrap strategic legacy capabilities](#phase-7-wrap-strategic-legacy-capabilities)
- [Phase 8: Redesign high-value interactions](#phase-8-redesign-high-value-interactions)
- [Phase 9: Validate through shadow and controlled cutover](#phase-9-validate-through-shadow-and-controlled-cutover)
- [Phase 10: Decommission deliberately](#phase-10-decommission-deliberately)
- [A strong first pilot](#a-strong-first-pilot)
- [Metrics that matter](#metrics-that-matter)
- [Interfaces retired](#interfaces-retired)
- [Business capabilities consolidated](#business-capabilities-consolidated)
- [Migration reuse rate](#migration-reuse-rate)
- [Redesign rate](#redesign-rate)
- [Cutover reconciliation accuracy](#cutover-reconciliation-accuracy)
- [Duplicate-transaction rate](#duplicate-transaction-rate)
- [Post-migration incident rate](#post-migration-incident-rate)
- [Error-resolution time](#error-resolution-time)
- [Consumer onboarding time](#consumer-onboarding-time)
- [Legacy-runtime reduction](#legacy-runtime-reduction)
- [Mapping duplication](#mapping-duplication)
- [Business monitoring coverage](#business-monitoring-coverage)
- [Common mistakes](#common-mistakes)
- [Mistake 1: Declaring every IDoc obsolete](#mistake-1-declaring-every-idoc-obsolete)
- [Mistake 2: Converting every file into a synchronous API](#mistake-2-converting-every-file-into-a-synchronous-api)
- [Mistake 3: Copying the old message structure into JSON](#mistake-3-copying-the-old-message-structure-into-json)
- [Mistake 4: Treating migration inventory as business analysis](#mistake-4-treating-migration-inventory-as-business-analysis)
- [Mistake 5: Rewriting undocumented logic without classifying it](#mistake-5-rewriting-undocumented-logic-without-classifying-it)
- [Mistake 6: Migrating unused interfaces](#mistake-6-migrating-unused-interfaces)
- [Mistake 7: Calling rehosting transformation](#mistake-7-calling-rehosting-transformation)
- [Mistake 8: Running old and new writers in parallel](#mistake-8-running-old-and-new-writers-in-parallel)
- [Mistake 9: Comparing only payloads](#mistake-9-comparing-only-payloads)
- [Mistake 10: Ignoring in-flight transactions at cutover](#mistake-10-ignoring-in-flight-transactions-at-cutover)
- [Mistake 11: Decommissioning before a peak business cycle](#mistake-11-decommissioning-before-a-peak-business-cycle)
- [Mistake 12: Keeping dual platforms permanently](#mistake-12-keeping-dual-platforms-permanently)
- [Mistake 13: Moving flows without migrating support procedures](#mistake-13-moving-flows-without-migrating-support-procedures)
- [Mistake 14: Assuming prebuilt content removes design work](#mistake-14-assuming-prebuilt-content-removes-design-work)
- [Mistake 15: Measuring success by number of interfaces converted](#mistake-15-measuring-success-by-number-of-interfaces-converted)
- [Questions architects and managers should ask](#questions-architects-and-managers-should-ask)
- [Modernization should remove uncertainty, not working capability](#modernization-should-remove-uncertainty-not-working-capability)
- [SAP legacy-integration modernization checklist](#sap-legacy-integration-modernization-checklist)
...
- Previous in the migration: [Why an API-First SAP Strategy Can Still Create a Point-to-Point Integration Mess](/blog/why-an-api-first-sap-strategy-can-still-create-a-point-to-point/)
- Next in the migration: [How to Design a Modern SAP Integration Architecture Without Creating a New Middleware Monolith](/blog/how-to-design-a-modern-sap-integration-architecture-without-creating-a/)
