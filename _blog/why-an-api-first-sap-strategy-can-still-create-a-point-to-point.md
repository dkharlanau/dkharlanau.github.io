---
layout: blog
title: "Why an API-First SAP Strategy Can Still Create a Point-to-Point Integration Mess"
description: "Old files, RFC connections, IDocs and direct database access should gradually disappear."
slug: why-an-api-first-sap-strategy-can-still-create-a-point-to-point
permalink: /blog/why-an-api-first-sap-strategy-can-still-create-a-point-to-point/
date: 2026-07-17
...
tags:
  ...
  - integration
canonical_url: https://dkharlanau.github.io/blog/why-an-api-first-sap-strategy-can-still-create-a-point-to-point/
status: needs_verification
verified: false
robots: noindex,follow
sitemap: false
reading_time_minutes: 30
migration_sequence: 26
...
- [API-first is a design principle, not a protocol policy](#api-first-is-a-design-principle-not-a-protocol-policy)
- [An API is not automatically reusable](#an-api-is-not-automatically-reusable)
- [APIs should expose capabilities, not tables](#apis-should-expose-capabilities-not-tables)
- [Not every SAP object needs a public API](#not-every-sap-object-needs-a-public-api)
- [Start with the interaction](#start-with-the-interaction)
- [Query](#query)
- [Command](#command)
- [Process initiation](#process-initiation)
- [Bulk transfer](#bulk-transfer)
- [The synchronous API trap](#the-synchronous-api-trap)
- [Do not wait synchronously for a long business process](#do-not-wait-synchronously-for-a-long-business-process)
- [API acceptance and business completion are different](#api-acceptance-and-business-completion-are-different)
- [Use stable business identities](#use-stable-business-identities)
- [Idempotency is part of API design](#idempotency-is-part-of-api-design)
- [Timeouts create uncertain outcomes](#timeouts-create-uncertain-outcomes)
- [Errors should be useful to the consumer](#errors-should-be-useful-to-the-consumer)
- [Separate temporary and permanent errors](#separate-temporary-and-permanent-errors)
- [Do not use HTTP codes as the complete business model](#do-not-use-http-codes-as-the-complete-business-model)
- [API composition can recreate the monolith outside SAP](#api-composition-can-recreate-the-monolith-outside-sap)
- [Orchestration requires ownership](#orchestration-requires-ownership)
- [API management is not process integration](#api-management-is-not-process-integration)
- [The gateway should protect and govern the contract](#the-gateway-should-protect-and-govern-the-contract)
- [An API proxy does not create an API product](#an-api-proxy-does-not-create-an-api-product)
- [API products need owners](#api-products-need-owners)
- [Business or domain owner](#business-or-domain-owner)
- [Technical product owner](#technical-product-owner)
- [Reuse should be measured, not assumed](#reuse-should-be-measured-not-assumed)
- [Avoid consumer-specific APIs where the business meaning is shared](#avoid-consumer-specific-apis-where-the-business-meaning-is-shared)
- [Separate system APIs, process APIs and experience APIs carefully](#separate-system-apis-process-apis-and-experience-apis-carefully)
- [Graph and unified access do not remove ownership](#graph-and-unified-access-do-not-remove-ownership)
- [Read APIs and write APIs have different risk](#read-apis-and-write-apis-have-different-risk)
- [Use purpose-specific authorization](#use-purpose-specific-authorization)
- [Rate limits are business protection](#rate-limits-are-business-protection)
- [Caching requires semantic care](#caching-requires-semantic-care)
- [APIs need service objectives](#apis-need-service-objectives)
- [Monitor consumer experience, not only gateway traffic](#monitor-consumer-experience-not-only-gateway-traffic)
- [API analytics should answer practical questions](#api-analytics-should-answer-practical-questions)
- [Version only when necessary, but version clearly](#version-only-when-necessary-but-version-clearly)
- [Do not maintain every version forever](#do-not-maintain-every-version-forever)
- [Consumer-driven contracts can expose hidden dependencies](#consumer-driven-contracts-can-expose-hidden-dependencies)
- [API documentation should explain business behaviour](#api-documentation-should-explain-business-behaviour)
- [Discovery matters](#discovery-matters)
- [SAP Business Accelerator Hub should be a starting point, not an automatic design decision](#sap-business-accelerator-hub-should-be-a-starting-point-not-an-automatic-design-decision)
- [Standard API versus custom API](#standard-api-versus-custom-api)
- [An API facade can protect consumers](#an-api-facade-can-protect-consumers)
- [Do not confuse abstraction with duplication](#do-not-confuse-abstraction-with-duplication)
- [APIs and events should work together](#apis-and-events-should-work-together)
- [APIs and files can also coexist](#apis-and-files-can-also-coexist)
- [AI agents change the API consumer model](#ai-agents-change-the-api-consumer-model)
- [Do not expose unrestricted CRUD as agent tools](#do-not-expose-unrestricted-crud-as-agent-tools)
- [Agent-ready does not mean agent-authorized](#agent-ready-does-not-mean-agent-authorized)
- [Build an API portfolio, not only APIs](#build-an-api-portfolio-not-only-apis)
- [Classify APIs by role](#classify-apis-by-role)
- [Core business APIs](#core-business-apis)
- [Internal operational APIs](#internal-operational-apis)
- [Partner APIs](#partner-apis)
- [Experience APIs](#experience-apis)
- [Agent-facing APIs and tools](#agent-facing-apis-and-tools)
- [Build APIs around domain boundaries](#build-apis-around-domain-boundaries)
- [A practical API design sequence](#a-practical-api-design-sequence)
- [Step 1: Define the business capability](#step-1-define-the-business-capability)
- [Step 2: Identify the domain owner](#step-2-identify-the-domain-owner)
- [Step 3: Choose synchronous or asynchronous behaviour](#step-3-choose-synchronous-or-asynchronous-behaviour)
- [Step 4: Define the business contract](#step-4-define-the-business-contract)
- [Step 5: Design idempotency and uncertainty handling](#step-5-design-idempotency-and-uncertainty-handling)
- [Step 6: Define authorization by purpose](#step-6-define-authorization-by-purpose)
- [Step 7: Decide what remains backend-specific](#step-7-decide-what-remains-backend-specific)
- [Step 8: Define lifecycle](#step-8-define-lifecycle)
- [Step 9: Test contracts and failure conditions](#step-9-test-contracts-and-failure-conditions)
- [Step 10: Monitor business outcomes](#step-10-monitor-business-outcomes)
- [Example: designing an order-submission API](#example-designing-an-order-submission-api)
- [Weak contract](#weak-contract)
- [Stronger contract](#stronger-contract)
- [Example: designing an availability API](#example-designing-an-availability-api)
- [Example: designing a supplier API](#example-designing-a-supplier-api)
- [A strong first API-first pilot](#a-strong-first-api-first-pilot)
- [Metrics that matter](#metrics-that-matter)
- [API reuse](#api-reuse)
- [Business-success rate](#business-success-rate)
- [Duplicate-prevention rate](#duplicate-prevention-rate)
- [Uncertain-outcome rate](#uncertain-outcome-rate)
- [Consumer onboarding time](#consumer-onboarding-time)
- [Breaking-change rate](#breaking-change-rate)
- [Version distribution](#version-distribution)
- [Business-error rate](#business-error-rate)
- [Backend-load concentration](#backend-load-concentration)
- [Unused-API rate](#unused-api-rate)
- [Agent-action exception rate](#agent-action-exception-rate)
- [Common mistakes](#common-mistakes)
- [Mistake 1: Defining API-first as REST-only](#mistake-1-defining-api-first-as-rest-only)
- [Mistake 2: Wrapping SAP internals in JSON](#mistake-2-wrapping-sap-internals-in-json)
- [Mistake 3: Building an API for every table or object](#mistake-3-building-an-api-for-every-table-or-object)
- [Mistake 4: Using synchronous APIs for long processes](#mistake-4-using-synchronous-apis-for-long-processes)
- [Mistake 5: Retrying commands without idempotency](#mistake-5-retrying-commands-without-idempotency)
- [Mistake 6: Returning only technical errors](#mistake-6-returning-only-technical-errors)
- [Mistake 7: Putting orchestration in the API gateway](#mistake-7-putting-orchestration-in-the-api-gateway)
- [Mistake 8: Forcing every consumer through one universal API](#mistake-8-forcing-every-consumer-through-one-universal-api)
- [Mistake 9: Creating consumer-specific versions of the same capability](#mistake-9-creating-consumer-specific-versions-of-the-same-capability)
- [Mistake 10: Publishing APIs without owners](#mistake-10-publishing-apis-without-owners)
- [Mistake 11: Keeping every version indefinitely](#mistake-11-keeping-every-version-indefinitely)
- [Mistake 12: Giving agents generic write APIs](#mistake-12-giving-agents-generic-write-apis)
- [Questions architects and managers should ask](#questions-architects-and-managers-should-ask)
- [The goal is controlled business access](#the-goal-is-controlled-business-access)
- [API-first SAP integration checklist](#api-first-sap-integration-checklist)
...
- Previous in the migration: [How to Design Event-Driven SAP Integrations Without Losing Control](/blog/how-to-design-event-driven-sap-integrations-without-losing-control/)
- Next in the migration: [How to Modernize SAP IDoc and File Integrations Without Rewriting the Entire Landscape](/blog/how-to-modernize-sap-idoc-and-file-integrations-without-rewriting-the/)
