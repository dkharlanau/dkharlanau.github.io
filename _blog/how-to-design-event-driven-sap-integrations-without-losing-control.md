---
layout: blog
title: "How to Design Event-Driven SAP Integrations Without Losing Control"
description: "a warehouse platform; - a customer portal; - a data platform; - a notification service; - an AI agent."
slug: how-to-design-event-driven-sap-integrations-without-losing-control
permalink: /blog/how-to-design-event-driven-sap-integrations-without-losing-control/
date: 2026-07-17
...
tags:
  ...
  - integration
canonical_url: https://dkharlanau.github.io/blog/how-to-design-event-driven-sap-integrations-without-losing-control/
status: needs_verification
verified: false
robots: noindex,follow
sitemap: false
reading_time_minutes: 31
migration_sequence: 25
...
- [Event-driven architecture is not simply asynchronous integration](#event-driven-architecture-is-not-simply-asynchronous-integration)
- [A business event describes something that has happened](#a-business-event-describes-something-that-has-happened)
- [A command asks for a future action](#a-command-asks-for-a-future-action)
- [Do not hide commands behind event names](#do-not-hide-commands-behind-event-names)
- [Technical events and business events are different](#technical-events-and-business-events-are-different)
- [Data-change events are not always business events](#data-change-events-are-not-always-business-events)
- [The first design question: what fact matters?](#the-first-design-question-what-fact-matters)
- [Publish meaningful state transitions](#publish-meaningful-state-transitions)
- [Event granularity matters](#event-granularity-matters)
- [Too broad](#too-broad)
- [Too narrow](#too-narrow)
- [Practical balance](#practical-balance)
- [An event is not automatically the full business object](#an-event-is-not-automatically-the-full-business-object)
- [Notification event](#notification-event)
- [Data-carrying event](#data-carrying-event)
- [A useful hybrid pattern](#a-useful-hybrid-pattern)
- [Events need stable business identifiers](#events-need-stable-business-identifiers)
- [Event identity supports duplicate protection](#event-identity-supports-duplicate-protection)
- [Business idempotency is stronger than event deduplication](#business-idempotency-is-stronger-than-event-deduplication)
- [Ordering is rarely global](#ordering-is-rarely-global)
- [Include object version or sequence](#include-object-version-or-sequence)
- [Not every consumer needs every intermediate state](#not-every-consumer-needs-every-intermediate-state)
- [Eventual consistency must be a business decision](#eventual-consistency-must-be-a-business-decision)
- [Do not promise immediate consistency through asynchronous events](#do-not-promise-immediate-consistency-through-asynchronous-events)
- [The dual-write problem](#the-dual-write-problem)
- [Do not publish before the business transaction is final](#do-not-publish-before-the-business-transaction-is-final)
- [Event publication needs monitoring](#event-publication-needs-monitoring)
- [Consumers are operational services](#consumers-are-operational-services)
- [Successful broker delivery is not successful business processing](#successful-broker-delivery-is-not-successful-business-processing)
- [Acknowledgement timing matters](#acknowledgement-timing-matters)
- [Retries must distinguish temporary and permanent failures](#retries-must-distinguish-temporary-and-permanent-failures)
- [Temporary failure](#temporary-failure)
- [Permanent business failure](#permanent-business-failure)
- [Unknown failure](#unknown-failure)
- [Dead-letter queues are not resolution](#dead-letter-queues-are-not-resolution)
- [Replay is a business operation](#replay-is-a-business-operation)
- [Separate state reconstruction from side effects](#separate-state-reconstruction-from-side-effects)
- [Event schema is a product contract](#event-schema-is-a-product-contract)
- [Avoid exposing internal table structures](#avoid-exposing-internal-table-structures)
- [Use bounded event domains](#use-bounded-event-domains)
- [Topic naming should not carry all business logic](#topic-naming-should-not-carry-all-business-logic)
- [Schema evolution requires compatibility rules](#schema-evolution-requires-compatibility-rules)
- [Additive does not always mean harmless](#additive-does-not-always-mean-harmless)
- [Consumers should tolerate unknown optional fields](#consumers-should-tolerate-unknown-optional-fields)
- [Create an event catalogue](#create-an-event-catalogue)
- [Do not require publishers to know every consumer](#do-not-require-publishers-to-know-every-consumer)
- [Consumer ownership must be explicit](#consumer-ownership-must-be-explicit)
- [Separate mandatory and optional consumers](#separate-mandatory-and-optional-consumers)
- [Event-driven does not mean orchestration-free](#event-driven-does-not-mean-orchestration-free)
- [Choreography versus orchestration](#choreography-versus-orchestration)
- [Choreography](#choreography)
- [Orchestration](#orchestration)
- [Avoid event chains with no owner](#avoid-event-chains-with-no-owner)
- [Correlation IDs support end-to-end visibility](#correlation-ids-support-end-to-end-visibility)
- [Observability should follow the business process](#observability-should-follow-the-business-process)
- [Measure event lag](#measure-event-lag)
- [Monitor consumer backlog age, not only volume](#monitor-consumer-backlog-age-not-only-volume)
- [Reconciliation remains necessary](#reconciliation-remains-necessary)
- [Do not depend on users to detect missing events](#do-not-depend-on-users-to-detect-missing-events)
- [Security should follow least data and least authority](#security-should-follow-least-data-and-least-authority)
- [Event retention is a business and compliance decision](#event-retention-is-a-business-and-compliance-decision)
- [AI agents make event governance more important](#ai-agents-make-event-governance-more-important)
- [Use events to trigger investigation before execution](#use-events-to-trigger-investigation-before-execution)
- [A reference SAP event architecture](#a-reference-sap-event-architecture)
- [1. Authoritative application](#1-authoritative-application)
- [2. Event publication layer](#2-event-publication-layer)
- [3. Event broker or mesh](#3-event-broker-or-mesh)
- [4. Consumer ingestion layer](#4-consumer-ingestion-layer)
- [5. Consumer business processing](#5-consumer-business-processing)
- [6. Verification layer](#6-verification-layer)
- [7. Reconciliation layer](#7-reconciliation-layer)
- [8. Governance layer](#8-governance-layer)
- [Example: sales order events](#example-sales-order-events)
- [Event 1: Sales order created](#event-1-sales-order-created)
- [Event 2: Sales order confirmed](#event-2-sales-order-confirmed)
- [Event 3: Sales order blocked](#event-3-sales-order-blocked)
- [Event 4: Sales order cancelled](#event-4-sales-order-cancelled)
- [Example: business partner distribution](#example-business-partner-distribution)
- [Example: goods movement and downstream action](#example-goods-movement-and-downstream-action)
- [Corrections should be events, not silent history edits](#corrections-should-be-events-not-silent-history-edits)
- [A practical rollout sequence](#a-practical-rollout-sequence)
- [Phase 1: Select one business event](#phase-1-select-one-business-event)
- [Phase 2: Define semantics](#phase-2-define-semantics)
- [Phase 3: Define delivery assumptions](#phase-3-define-delivery-assumptions)
- [Phase 4: Create the event contract](#phase-4-create-the-event-contract)
- [Phase 5: Build one controlled consumer](#phase-5-build-one-controlled-consumer)
- [Phase 6: Add reconciliation](#phase-6-add-reconciliation)
- [Phase 7: Test failure conditions](#phase-7-test-failure-conditions)
- [Phase 8: Add more consumers](#phase-8-add-more-consumers)
- [Phase 9: Introduce catalogue and governance](#phase-9-introduce-catalogue-and-governance)
- [Phase 10: Expand by domain](#phase-10-expand-by-domain)
- [A strong first pilot](#a-strong-first-pilot)
- [Metrics that matter](#metrics-that-matter)
- [Event-publication completeness](#event-publication-completeness)
- [Duplicate-processing rate](#duplicate-processing-rate)
- [Out-of-order handling rate](#out-of-order-handling-rate)
- [Consumer lag](#consumer-lag)
- [Dead-letter age](#dead-letter-age)
- [Replay success](#replay-success)
- [Schema compatibility](#schema-compatibility)
- [Business-completion rate](#business-completion-rate)
- [Silent-loss detection time](#silent-loss-detection-time)
- [Event-related incident rate](#event-related-incident-rate)
- [Common mistakes](#common-mistakes)
- [Mistake 1: Calling every asynchronous message an event](#mistake-1-calling-every-asynchronous-message-an-event)
- [Mistake 2: Publishing technical changes as business facts](#mistake-2-publishing-technical-changes-as-business-facts)
- [Mistake 3: Assuming exactly-once business processing](#mistake-3-assuming-exactly-once-business-processing)
- [Mistake 4: Assuming events arrive in order](#mistake-4-assuming-events-arrive-in-order)
- [Mistake 5: Ignoring the dual-write problem](#mistake-5-ignoring-the-dual-write-problem)
- [Mistake 6: Acknowledging before durable receipt](#mistake-6-acknowledging-before-durable-receipt)
- [Mistake 7: Retrying permanent business errors](#mistake-7-retrying-permanent-business-errors)
- [Mistake 8: Treating dead-letter storage as resolution](#mistake-8-treating-dead-letter-storage-as-resolution)
- [Mistake 9: Replaying events without controlling side effects](#mistake-9-replaying-events-without-controlling-side-effects)
- [Mistake 10: Changing schemas without consumer governance](#mistake-10-changing-schemas-without-consumer-governance)
- [Mistake 11: Measuring only broker health](#mistake-11-measuring-only-broker-health)
- [Mistake 12: Building event chains with no end-to-end owner](#mistake-12-building-event-chains-with-no-end-to-end-owner)
- [Questions architects and managers should ask](#questions-architects-and-managers-should-ask)
- [The goal is controlled asynchronous independence](#the-goal-is-controlled-asynchronous-independence)
- [Event-driven SAP integration checklist](#event-driven-sap-integration-checklist)
...
- Previous in the migration: [Where Should Mapping Logic Live in Modern SAP Integrations?](/blog/where-should-mapping-logic-live-in-modern-sap-integrations/)
- Next in the migration: [Why an API-First SAP Strategy Can Still Create a Point-to-Point Integration Mess](/blog/why-an-api-first-sap-strategy-can-still-create-a-point-to-point/)
